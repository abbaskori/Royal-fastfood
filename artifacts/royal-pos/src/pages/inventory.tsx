...
import { useMenuItems, useCategories } from "@/hooks/use-data";
          ...
          <div className="flex items-center gap-3">
            {lowStockCount > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 rounded-lg text-sm font-bold border border-red-100">
                <AlertTriangle className="w-4 h-4" /> {lowStockCount} items low on stock
              </div>
            )}
            <div className="px-4 py-2 bg-white rounded-xl shadow-sm border border-gray-200 text-sm font-bold text-gray-700 flex items-center gap-2">
              <ArrowRightLeft className="w-4 h-4 text-gray-400" />
              Total Tracked: {trackedItems.length}
            </div>
          </div>
        </div>
